import { BookOpen, Users, Star, Mail, Calendar, Loader2 } from "lucide-react";
    ...
    return (<div className="max-w-4xl mx-auto">
      ...
      <div className="bg-card border border-border rounded-2xl p-6 mb-8">
        <div className="flex items-start gap-6">
          <div className="w-24 h-24 rounded-2xl bg-primary flex items-center justify-center">
            {user?.avatar ? (<img src={user.avatar} alt={user.name} className="w-full h-full rounded-2xl object-cover"/>) : (<span className="text-3xl font-bold text-primary-foreground">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-card border border-border rounded-xl p-5">
          ...
          {skillsLearned.length === 0 ? (<p className="text-sm text-muted-foreground">No skills learned yet</p>) : (<div className="space-y-3">
              {skillsLearned.map((skill) => (<div key={skill._id} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
        ...
        <div className="bg-card border border-border rounded-xl p-5">
          ...
          {isLoadingTaught ? (<div className="flex justify-center py-4">
              <Loader2 className="w-6 h-6 animate-spin text-primary"/>
            </div>) : skillsTaught.length === 0 ? (<p className="text-sm text-muted-foreground">No skills taught yet</p>) : (<div className="space-y-3">
              {skillsTaught.map((skill) => (<div key={skill._id} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                  <span className="text-sm font-medium text-foreground">{skill.title}</span>
                  <div className="flex items-center gap-1 text-xs text-muted-foreground">
                    <Calendar className="w-3 h-3"/>
                    <span>{formatDate(skill.createdAt)}</span>
